import type { Metadata } from 'next';
import Link from 'next/link';
import { notFound } from 'next/navigation';

import { ArticleSchema } from '@/components/article-schema';
import { BreadcrumbSchema } from '@/components/breadcrumb-schema';
import { CtaCard } from '@/components/cta-card';
import { LegacyContent } from '@/components/legacy-content';
import { copy, resolveLocale } from '@/lib/i18n';
import { getLegacyDocument } from '@/lib/legacy-loader';
import { getRelatedRouteLinks } from '@/lib/route-index';
import { getStateOrNull, serviceStateCopy } from '@/lib/phase2-content';
import { extractStateSlug } from '@/lib/phase2-routes';
import { localizedPath } from '@/lib/routes';
import { createMetadata } from '@/lib/seo';
import { getManagedPageCopyWithFallback } from '@/lib/site-cms-content';
import { getServicesSlugs, getStaticLocales } from '@/lib/static-export';

type ServicesStateManagedCopy = {
  eyebrow: string;
};

const servicesStateFallback: ServicesStateManagedCopy = {
  eyebrow: 'State services',
};

export function generateStaticParams() {
  return getStaticLocales().flatMap((locale) => getServicesSlugs(locale).map((slug) => ({ locale, slug })));
}

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}): Promise<Metadata> {
  const { locale: rawLocale, slug } = await params;
  const locale = resolveLocale(rawLocale);
  const legacy = await getLegacyDocument(locale, ['services', slug]);

  if (legacy) {
    return createMetadata({
      locale,
      pathname: `/${locale}/services/${slug}`,
      title: legacy.title,
      description: legacy.description,
    });
  }

  const stateSlug = extractStateSlug('services', slug);
  const state = stateSlug ? getStateOrNull(stateSlug) : null;

  if (!state) {
    return createMetadata({
      locale,
      pathname: `/${locale}/services/${slug}`,
      title: 'State Services',
      description: 'State services page.',
    });
  }

  const t = serviceStateCopy(locale, state);

  return createMetadata({
    locale,
    pathname: `/${locale}/services/${slug}`,
    title: `${t.title} | ${copy[locale].brand}`,
    description: t.subtitle,
  });
}

export default async function ServicesStatePage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale: rawLocale, slug } = await params;
  const locale = resolveLocale(rawLocale);
  const pageCopy = getManagedPageCopyWithFallback<ServicesStateManagedCopy>(locale, 'servicesStatePage', servicesStateFallback);
  const nav = copy[locale].nav;
  const legacy = await getLegacyDocument(locale, ['services', slug]);

  if (legacy) {
    const relatedLinks = await getRelatedRouteLinks(locale, `services/${slug}`, 16);
    return (
      <>
        <BreadcrumbSchema
          items={[
            { name: nav.home, href: `/${locale}` },
            { name: nav.services, href: `/${locale}/services` },
            { name: legacy.heading, href: `/${locale}/services/${slug}` },
          ]}
        />
        <ArticleSchema
          locale={locale}
          pathname={`/${locale}/services/${slug}`}
          headline={legacy.heading}
          description={legacy.description}
          section="Services"
          keywords={[slug.replace(/^immigrate-to-/, '').replace(/-/g, ' '), 'Brazil immigration services']}
        />
        <LegacyContent locale={locale} document={legacy} slug={`services/${slug}`} relatedLinks={relatedLinks} />
      </>
    );
  }

  const stateSlug = extractStateSlug('services', slug);
  if (!stateSlug) notFound();

  const state = getStateOrNull(stateSlug);
  if (!state) notFound();

  const t = serviceStateCopy(locale, state);

  return (
    <>
      <BreadcrumbSchema
        items={[
          { name: nav.home, href: `/${locale}` },
          { name: nav.services, href: `/${locale}/services` },
          { name: t.title, href: `/${locale}/services/${slug}` },
        ]}
      />
      <ArticleSchema
        locale={locale}
        pathname={`/${locale}/services/${slug}`}
        headline={t.title}
        description={t.subtitle}
        section="Services"
        keywords={[state.slug.replace(/-/g, ' '), 'Brazil migration']}
      />
      <section className="border-b border-sand-200 bg-white">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-civic-700">{pageCopy.eyebrow}</p>
          <h1 className="mt-4 font-display text-5xl text-ink-900">{t.title}</h1>
          <p className="mt-6 max-w-3xl text-lg text-ink-700">{t.subtitle}</p>
        </div>
      </section>

      <section className="bg-sand-50">
        <div className="mx-auto grid max-w-7xl gap-5 px-4 py-14 sm:px-6 lg:grid-cols-3 lg:px-8">
          {t.modules.map((module) => (
            <article key={module.title} className="rounded-2xl border border-sand-200 bg-white p-6 shadow-sm">
              <h2 className="font-display text-2xl text-ink-900">{module.title}</h2>
              <p className="mt-3 text-sm text-ink-700">{module.detail}</p>
            </article>
          ))}
        </div>

        <div className="mx-auto max-w-7xl px-4 pb-14 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-3">
            <Link
              href={localizedPath(locale, '/services')}
              className="rounded-full border border-ink-300 bg-white px-5 py-2.5 text-sm font-semibold text-ink-900"
            >
              {copy[locale].nav.services}
            </Link>
            <Link
              href={localizedPath(locale, '/contact')}
              className="rounded-full bg-ink-900 px-5 py-2.5 text-sm font-semibold text-sand-50"
            >
              {copy[locale].cta.button}
            </Link>
          </div>
        </div>
      </section>

      <CtaCard locale={locale} />
    </>
  );
}
