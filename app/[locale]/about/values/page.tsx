...
const aboutValuesFallback: AboutValuesManagedCopy = {
  ...
  primaryButtonLabel: 'Read mission',
  secondaryButtonLabel: 'Read story',
  values: [
    {
      title: 'Clarity',
  ...
  commitmentsTitle: 'Ethical commitments',
  commitments: [
...
export default async function AboutValuesPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: rawLocale } = await params;
  const locale = resolveLocale(rawLocale);
  const pageCopy = getManagedPageCopyWithFallback<AboutValuesManagedCopy>(locale, 'aboutValuesPage', aboutValuesFallback);
      ...
      <section className="bg-sand-50">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          ...
          <article className="mt-8 rounded-2xl border border-sand-200 bg-white p-6">
            <h2 className="font-display text-2xl text-ink-900">{pageCopy.commitmentsTitle}</h2>
            <ul className="mt-4 grid gap-2 text-sm text-ink-700">
              {pageCopy.commitments.map((item) => (
