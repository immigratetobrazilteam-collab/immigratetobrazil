...
function validateOptionalHybridFields(relativePath, page) {
  if (page.contentSources != null) {
    assert(Array.isArray(page.contentSources) && page.contentSources.length >= 1, `${relativePath} contentSources must be a non-empty array`);
    for (const [index, source] of page.contentSources.entries()) {
      assert(isObject(source), `${relativePath} contentSources[${index}] must be an object`);
      for (const key of ['id', 'title', 'url']) {
        assert(isNonEmptyString(source[key]), `${relativePath} contentSources[${index}].${key} is required`);
      }
    }
  }

  if (page.factuality != null) {
    assert(isObject(page.factuality), `${relativePath} factuality must be an object`);
    for (const key of ['lastVerifiedAt', 'verificationLevel', 'confidence']) {
      assert(isNonEmptyString(page.factuality[key]), `${relativePath} factuality.${key} is required`);
    }
  }

  if (page.editorial != null) {
    assert(isObject(page.editorial), `${relativePath} editorial must be an object`);
    for (const key of ['primaryIntent', 'targetPersona']) {
      assert(isNonEmptyString(page.editorial[key]), `${relativePath} editorial.${key} is required`);
    }
  }

  if (page.seoV2 != null) {
    assert(isObject(page.seoV2), `${relativePath} seoV2 must be an object`);
    assert(isNonEmptyString(page.seoV2.primaryKeyword), `${relativePath} seoV2.primaryKeyword is required`);
    assert(Array.isArray(page.seoV2.secondaryKeywords), `${relativePath} seoV2.secondaryKeywords must be an array`);
    assert(isNonEmptyString(page.seoV2.searchIntent), `${relativePath} seoV2.searchIntent is required`);
    assert(isNonEmptyString(page.seoV2.snippetAnswer), `${relativePath} seoV2.snippetAnswer is required`);
  }
}
...
function validatePageShape(relativePath, page) {
  ...
  validateOptionalHybridFields(relativePath, page);
}
