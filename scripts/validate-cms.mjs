...
function validateManagedPagesValue(relativePath, fieldPath, value, template) {
  ...
  const optionalHybridKeys = new Set(['contentSources', 'factuality', 'editorial', 'seoV2']);
  for (const key of Object.keys(template)) {
    if (optionalHybridKeys.has(key) && !Object.prototype.hasOwnProperty.call(value, key)) {
      continue;
    }
    assert(
