  ...
  if (segments[0] === "about") {
    if (route === "/about/") {
      return {
        pageType: "AboutPage",
        pageKind: "about-hub",
        mainEntityKind: "person",
        collectionSource: "direct-children"
      };
    }

    if (route === "/about/lawyer/" || route === "/about/about/") {
      return {
        pageType: "AboutPage",
        pageKind: route === "/about/lawyer/" ? "lawyer" : "about",
        mainEntityKind: "person"
      };
    }

    return { pageType: "AboutPage", pageKind: "about", mainEntityKind: "person" };
  }
...
function buildOrganizationSchema(about) {
  return {
    ...
    description:
      ...
      "Immigrate to Brazil is the immigration-focused legal platform led by Monique Fernandes for visas, residency, citizenship, compliance, and cross-border Brazil planning.",
    email: about.runtime?.contact?.email || "moniquefadv@gmail.com",
    telephone: about.runtime?.contact?.phone || "+55 43 9961-4034",
    logo: `${SITE_DOMAIN}/assets/logo/logo.png`,
    founder: ref(PERSON_MONIQUE_ID),
    employee: [ref(PERSON_MONIQUE_ID)],
...
function buildLegalPracticeSchema(route, classification) {
  const practice = {
    ...
    name: "Monique Fernandes Brazil Immigration Attorney And Legal Services",
    description:
      "Attorney-led Brazil immigration legal services for visas, residencies, citizenship, strategy, compliance, and cross-border planning.",
    provider: ref(ORGANIZATION_ID),
    employee: [ref(PERSON_MONIQUE_ID)],
    areaServed: ref(BRAZIL_ID),
    availableLanguage: AVAILABLE_LANGUAGES,
    serviceType: "Brazil immigration attorney legal services",
    knowsAbout: [
    ...
    audience: [
      { "@type": "Audience", audienceType: "International individuals" },
      { "@type": "Audience", audienceType: "International families" },
      { "@type": "Audience", audienceType: "International companies" }
    ],
...
function buildMoniqueSchema(route, page) {
  const isPrimaryMoniquePage = route.startsWith("/about/");
  const person = {
    "@type": "Person",
    ...
    url: `${SITE_DOMAIN}/about/lawyer/`,
    jobTitle: "Brazilian immigration attorney",
    description:
      isPrimaryMoniquePage
        ? page.meta?.description || "Brazilian immigration attorney supporting cross-border immigration and residency matters."
        : "Brazilian immigration attorney supporting cross-border immigration and residency matters.",
    ...
    knowsAbout: [
      ...
      "Brazil residencies",
      "Brazilian citizenship and naturalisation",
      "Immigration defense and regularization",
      "Cross-border documentation and consular support"
    ],
    hasCredential: {
      "@type": "EducationalOccupationalCredential",
      name: "OAB/BAR registered attorney",
      credentialCategory: "Professional legal registration in Brazil",
      description: "Brazilian attorney with professional BAR and OAB registration."
    }
  };

  if (isPrimaryMoniquePage) {
...
function shouldIncludeMonique(route, bodyHtml) {
  return (
    ...
    /Monique Fernandes|Brazilian immigration attorney|OAB\/BAR|BAR\/OAB|OAB-registered|OAB registered/i.test(bodyHtml || "")
  );
}
