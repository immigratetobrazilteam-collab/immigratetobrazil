...
MONIQUE_HOME_PORTRAIT = "/assets/images/pages/process/shared/next-step/monique-fernandes-brazil-immigration-attorney-process-next-step-portrait.png"
...
def build_lookup(existing: dict[str, dict], preview_pages: dict[str, Page]) -> dict[str, str]:
    lookup: dict[str, str] = {
        ...
        "process filing": "/process/filing/",
        "process approval": "/process/approval/",
        "process aftercare": "/process/aftercare/",
        ...
        "process mistakes": "/process/mistakes/",
        "process alone": "/process/alone/",
...
def existing_page_title(route: str, existing: dict[str, dict], page: Page) -> str:
    if route.startswith("/process/"):
        if route == "/process/":
            return "Brazil Immigration Process"
...
def display_process_label(label: str) -> str:
    ...
    if cleaned.startswith("Process ") and cleaned != "Process Hub":
        return cleaned[len("Process ") :]
    return cleaned
...
def render_hero(page: Page, family: str, image: dict) -> str:
    badges = page.links[:6]
    badge_html = "".join(
        f'<span class="hero-badge"><span class="hero-badge__icon" aria-hidden="true">{COMPASS_ICON}</span><span>{html.escape(display_process_label(label) if family == "process" else clean_text(label))}</span></span>'
        for label in badges
    )
...
<div class="hero-meta">
        ...
        <div class="hero-panel hero-panel--signals">
          ...
          <ul class="hero-panel-list">
            {''.join(f'<li class="hero-panel-item"><span class="hero-panel-item__icon" aria-hidden="true">{COMPASS_ICON}</span><span>{html.escape(display_process_label(label) if family == "process" else clean_text(label))}</span></li>' for label in badges[:2])}
          </ul>
        </div>
      </div>
    </div>
...
def render_body(page: Page, preview_map: dict[str, Page], existing: dict[str, dict], route_lookup: dict[str, str]) -> str:
    ...
    brand_title = "Immigrate to Brazil"
    brand_copy = "Structured guidance for immigration, relocation, long-term planning, and calmer decisions about Brazil."
    brand_media = f'<img class="brand-wordmark__mark" src="{LOGO_MAIN}" alt="{html.escape(logo_alt)}" width="72" height="72" loading="lazy" decoding="async" />'
    consultation_copy = "Use consultation for route comparison, chronology review, document planning, and clearer next-step guidance."
    if family == "process":
        brand_title = "Monique Fernandes"
        brand_copy = "Brazilian immigration attorney guiding consultation, assessment, filing, approval, and aftercare for clients in Brazil and abroad."
        brand_media = (
            f'<img src="{MONIQUE_HOME_PORTRAIT}" alt="Monique Fernandes, Brazilian immigration attorney serving clients across Brazil and remotely" '
            'width="640" height="640" loading="lazy" decoding="async" '
            'style="display:block;width:100%;height:260px;object-fit:contain;object-position:center bottom;border-radius:1rem;margin-bottom:0.95rem;background:linear-gradient(180deg,rgba(16,32,54,.96),rgba(9,20,34,.96));padding:0.4rem 0.4rem 0;" />'
        )
        consultation_copy = "Book Monique for route analysis, chronology review, document planning, and a clearer next step for immigration to Brazil."
    cta = f"""
  <section class="lead-form-block" id="consultation-form">
    ...
    <div class="card-grid compact">
      <article class="info-card">
        {brand_media}
        <h3>{html.escape(brand_title)}</h3>
        <p>{html.escape(brand_copy)}</p>
      </article>
      <article class="info-card">
        <h3>Book consultation</h3>
        <p>{html.escape(consultation_copy)}</p>
...
def meta_title(page: Page, family: str, existing: dict[str, dict]) -> str:
    if family == "process":
        ...
        if page.path == "/process/":
            return "Brazil Immigration Process With Monique Fernandes | Immigrate to Brazil"
        return f"{runtime_title} With Monique Fernandes | Brazil Immigration Process"
