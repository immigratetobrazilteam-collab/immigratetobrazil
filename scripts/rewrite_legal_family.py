...
CONTACT_PORTRAIT = "/assets/images/pages/legal/shared/contact-card/monique-fernandes-brazil-immigration-attorney-legal-contact-portrait.webp"
...
SEO_TITLE_MAP = {
    "/legal/": "Legal Notices | Monique Fernandes | Immigrate to Brazil",
    "/legal/privacy/": "Privacy Policy | Monique Fernandes | Immigrate to Brazil",
    "/legal/cookies/": "Cookies Policy | Monique Fernandes | Immigrate to Brazil",
    "/legal/terms/": "Terms & Conditions | Monique Fernandes | Immigrate to Brazil",
    "/legal/payment/": "Payment Terms | Monique Fernandes | Immigrate to Brazil",
    "/legal/refund/": "Refund Policy | Monique Fernandes | Immigrate to Brazil",
    "/legal/form/": "Form & Intake Policy | Monique Fernandes | Immigrate to Brazil",
    "/legal/gdpr/": "GDPR Notice | Monique Fernandes | Immigrate to Brazil",
    "/legal/lgpd/": "LGPD Notice | Monique Fernandes | Immigrate to Brazil",
    "/legal/accessibility/": "Accessibility Statement | Monique Fernandes | Immigrate to Brazil",
    "/legal/disclaimer/": "Disclaimer & Legal Notice | Monique Fernandes | Immigrate to Brazil",
    "/legal/emergency/": "Emergency Resources | Monique Fernandes | Immigrate to Brazil",
    "/legal/search/": "Search | Monique Fernandes | Immigrate to Brazil",
    "/legal/404/": "Page Not Found | Monique Fernandes | Immigrate to Brazil",
}


def title_suffix(title: str, route: str) -> str:
    return SEO_TITLE_MAP.get(route, f"{title} | Monique Fernandes | Immigrate to Brazil")
...
def render_inline(value: str) -> str:
    ...
    escaped = escaped.replace(
        PHONE,
        f'<a href="https://api.whatsapp.com/send/?phone=554399614034&text=Hello+I+would+like+to+talk+to+attorney+Monique&type=phone_number&app_absent=0">{PHONE}</a>',
    )
    return escaped
...
def clean_browser_title(value: str) -> str:
    value = clean_text(value)
    return re.sub(r"\s+\|\s+Immigrate to Brazil$", "", value).strip()
...
def route_title_for_related(route: str, preview_pages: dict[str, dict], existing: dict[str, dict]) -> tuple[str, str]:
    ...
    if route in existing:
        page = existing[route]
        title = clean_browser_title(page.get("meta", {}).get("title", "")) or page.get("runtime", {}).get("pageTitle")
...
def hero_action_href(label: str, route: str) -> str:
    mapping = {
        ...
        "Submit a privacy request": f"mailto:{EMAIL}",
        ...
        "Payment support": f"mailto:{EMAIL}",
        "Open refund policy": "/legal/refund/",
        "Request refund review": f"mailto:{EMAIL}",
        ...
        "Send LGPD request": f"mailto:{EMAIL}",
        "Report accessibility issue": f"mailto:{EMAIL}",
        ...
        "Submit privacy request": f"mailto:{EMAIL}",
        "Read disclaimer": "/legal/disclaimer/",
        "Send payment enquiry": f"mailto:{EMAIL}",
    }
...
<div class="hero-panel hero-panel--signals">
          <strong>What clients should know</strong>
...
def render_faq(page: dict) -> str:
  ...
  <section class="faq-block" id="faq" data-faq="true">
    <div class="section-head">
      ...
      <p>Short answers about how Monique Fernandes handles this topic and what clients usually need to know before proceeding.</p>
    </div>
  ...
  <section class="lead-form-block" id="consultation-form">
    ...
    <div class="card-grid compact">
      <article class="info-card">
        <img src="{CONTACT_PORTRAIT}" alt="Monique Fernandes, Brazilian immigration attorney, ready to help with consultation and client support." width="320" height="320" loading="lazy" decoding="async" style="display:block;width:120px;height:120px;object-fit:cover;border-radius:999px;margin:0 auto 1rem;" />
        <h3>Monique Fernandes</h3>
        <p>Brazilian immigration attorney handling client intake, privacy, payment, and service stages with clear boundaries and practical guidance.</p>
      </article>
      <article class="info-card">
        <h3>Book consultation</h3>
        <p>Move from general policy information into Monique's case-specific review of your route, documents, timing, and next step.</p>
        <p><a class="btn btn-cta btn-sm" href="/start-consultation/" data-cta-click="true">Start Consultation</a></p>
      </article>
      <article class="info-card">
        <h3>Contact on WhatsApp</h3>
        <p>Use WhatsApp for operational clarification, urgency signaling, or help reaching the right Monique Fernandes service page.</p>
...
def build_shell(page: dict, preview_pages: dict[str, dict], existing: dict[str, dict]) -> dict:
    ...
    if page["route"] == "/legal/":
        breadcrumbs.append({"label": "Legal", "current": True})
    elif page["route"].startswith("/legal/"):
        breadcrumbs.append({"label": "Legal", "href": "/legal/"})
        breadcrumbs.append({"label": title, "current": True})
    else:
        breadcrumbs.append({"label": title, "current": True})
    shell = {
        ...
        "sidebar": {
            ...
            "nextStep": {
                "lead": "Use consultation if you want Monique Fernandes to apply this policy or notice to your own immigration matter. Use WhatsApp for operational clarification or routing support.",
                "actions": [
                    {
                ...
                "note": "Representation, filings, refunds, privacy requests, and timing are all handled according to the actual service stage, the facts of the matter, and applicable law.",
            },
        },
