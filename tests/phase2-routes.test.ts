...
import { buildFaqStateSlug, extractStateSlug, isPolicySlug, routePatternFromSlug } from '../lib/phase2-routes';
...
describe('phase2-routes', () => {
  it('extracts valid state slugs by route type', () => {
    ...
    expect(extractStateSlug('faq', buildFaqStateSlug('rio-grande-do-sul'))).toBe('rio-grande-do-sul');
  ...
  it('maps route patterns from slug', () => {
    ...
    expect(routePatternFromSlug(buildFaqStateSlug('acre'))).toBe('faq');
